...
и в данный момент времени, это значит, что и все коэффициенты в этих ДУ относятся к данной точке
пространства и к данному моменту времени. С учетом этого факта можно обнаружить, что, например,
коэффициент полной сжимаемости зависит от насыщенности в данный момент времени, а значит нельзя
сразу решить уравнение на фильтрацию, поскольку насыщенность в этот момент ещё неизвестна. В то же
время насыщенность сама зависит от давления в данный момент времени, а значит итоговое уравнение
фильтрации становится нелинейным: коэффициенты уравнения сами зависят от искомого давления.\\
...
\end{multline}
В уравнении \refeq{eq:FiltrationLinear} уже видно, что насыщенность, например, линеаризована по 
методу Пикара, т.е. принимается известной и постоянной (значение коэффициента берется на 
итерации k для поиска решения на итерации k+1).
